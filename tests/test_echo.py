...
class TestEcho(unittest.TestCase):
    ...
    def test_parser_namespace(self):
        result = self.module.create_parser()
        text_args = ['Hello', '-l', '-u', '-t']
        ns = result.parse_args(text_args)
        self.assertTrue(ns.lower, 'parser does not except -l')
        self.assertTrue(ns.upper, 'parser does not except -u')
        self.assertTrue(ns.title, 'parser does not except -t')
